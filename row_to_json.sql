...
CREATE PROCEDURE ROW_TO_JSON (
	...
	SET @JSON_PRAM = (SELECT GROUP_CONCAT(CONCAT("'", column_name, "',", column_name)) FROM information_schema.columns WHERE table_name = _table_name);
...
CALL ROW_TO_JSON("table", "id", "31415926535");
